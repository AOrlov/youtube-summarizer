...
def test_index_renders_configured_model_name_label(client):
    response = client.get("/")

    html = response.get_data(as_text=True)

    assert response.status_code == 200
    assert "Generated by gemini-2.0-flash" in html
    assert 'id="summaryModelLabel"' in html
    assert 'id="resummarizeBtn"' in html
    assert "force_regenerate" in html
    assert "Generated with Gemini when available." not in html
...
def test_api_summarize_forwards_force_regenerate(client, web_module):
    response = client.post(
        "/api/summarize",
        json={
            "video_url": "https://youtube.com/watch?v=dQw4w9WgXcQ",
